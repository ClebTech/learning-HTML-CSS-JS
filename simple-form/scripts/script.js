function myFunction(){
    ...
    function recebeEventoForm (evento){ //função que recebe o objeto chamado 'evento', que é o objeto de evento gerado automaticamente pelo navegador quando o formulário é enviado, nesse objeto há dados como: o tipo de evento (submit), o elemento que ativou o evento (formulario), e metodos para manipular o evento (preventDefault). Ou seja, quando o usuario clica para enviar o formulario... perceba que o 'evento', neste contexto, é o envio do formulário, que ocorre quando o botão de submit é acionado.
    ...
    formu.addEventListener('submit', recebeEventoForm); //dentro da tag <form> há um botão de submit, aqui estou dizendo que quando esse botão de submit for acionado, a função 'recebeEventoFrom' deverá ser executada... eu não coloco paranteses na função, pois quero que a função seja executada apenas quando o botão submit for acionado, caso eu coloque parenteses, a função será executada imediatamente, mesmo se o usuario nao clicar em submit e isso resultaria em um erro. Perceba que, quando eu ponho parenteses na frente da função eu estou passando o resultado que função retorna e nao a função em si, quando eu não coloco parenteses, eu estou passando a propria função como referencia
}
